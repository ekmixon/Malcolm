...
def run_process(
    ...
    try:
        ...
        retcode, cmdout, cmderr = check_output_input(
            command, input=stdin.encode() if (PY3 and stdin) else stdin, cwd=cwd, env=env
        )
...
class Beatbox(object):
    ...
    def run_beat_command(self, command, stdout=True, stderr=True, stdin=None, retry=0, retrySleepSec=5):
        return self.run_process(
            self.build_beat_command(command),
            stdout=stdout,
            stderr=stderr,
            stdin=stdin,
            retry=retry,
            ...
            if YesOrNo(
                ...
                while tmpVal not in ['http', 'https']:
                    ...
                    if len(tmpVal) == 0:
                        tmpVal = tmpDefault
                self.keystoreItems[
                    BEAT_OS_PROTOCOL.replace('_OS_', '_KIBANA_' if (destination == 'Kibana') else '_OS_')
                ] = tmpVal
                ...
                while tmpVal not in ['none', 'full']:
                    ...
                    if len(tmpVal) == 0:
                        tmpVal = tmpDefault
                self.keystoreItems[
                    BEAT_OS_SSL_VERIFY.replace('_OS_', '_KIBANA_' if (destination == 'Kibana') else '_OS_')
                ] = tmpVal
